...
certificateModal.addEventListener('show.bs.modal', function (event) {
  ...
  const pdfUrl = triggerLink.getAttribute('data-pdf');

  // Add #zoom=page-fit only if not already present
  const hasHash = pdfUrl.includes('#');
  const fitToPageUrl = hasHash ? pdfUrl : `${pdfUrl}#zoom=page-fit`;

  pdfViewer.src = fitToPageUrl;
});
